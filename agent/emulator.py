...
class EmulatorHarness:
    def __init__(self, rom_path: str):
        ...
        self.timing_params = ActionTimingParams()
    ...
    def _load_knowledge_base(self) -> Dict:
        try:
            ...
            logger.warning("Knowledge base not found at %s", CFG.KNOWLEDGE_BASE_FILE)
    ...
    def _get_screenshot(self) -> Image.Image:
        try:
            ...
            logger.exception("Unable to capture screenshot: %s", e)
    ...
    def _read_ram_data(self) -> Dict[str, int]:
        ...
        try:
            ...
            logger.exception("Unable to read memory: %s", e)
    ...
    def _parse_state(self, ram_data: Dict, screenshot: Image.Image) -> Dict[str, Any]:
        parsed = {
            "location": f"Map {ram_data.get('current_map', 0)} at ({ram_data.get('player_x', 0)}, {ram_data.get('player_y', 0)})",
    ...
    def press_button(self, button: str):
        try:
            ...
            logger.debug("Button: %s hold=%s wait=%s type=%s", button, hold_frames, wait_frames, action_type)
            if bool(CFG.RENDER_ENABLED) and (not bool(CFG.HEADLESS)):
                time.sleep(0.01)
        except Exception as e:
            logger.exception("Unable to press button %s: %s", button, e)
    ...
    def search_knowledge_base(self, query: str) -> Optional[str]:
        if query.startswith("type_matchup:"):
            ...
            return self.loaded_knowledge_base.get("type_matchups", {}).get(pokemon_type, "Unknown")
        return None

    def solve_boulder_puzzle(self, puzzle_id: str) -> bool:
        logger.info("Solving puzzle: %s", puzzle_id)
    ...
    def close(self):
        if self.emulator:
            try:
                ...
                logger.exception("Unable to close emulator: %s", e)
